...
test.group('Knowledgebase', (group) => {
  group.tap((test) => test.tags(['@Knowledgebase']))
  ...
  test('should return 201 when duplicate knowledgeBase', async ({ client }) => {
    const response = await client
      .post(`api/v1/knowledge/duplicate/${knowledgebase.id}`)
      .header('Authorization', 'Bearer ' + token)
  ...
  test('should return 422 when no knowledgebase id provide', async ({ client }) => {
    const response = await client
      .post('api/v1/knowledge/duplicate/:id')
      .header('Authorization', 'Bearer ' + token)
    response.assertStatus(422)
  })
